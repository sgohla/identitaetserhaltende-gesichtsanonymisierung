# Pipeline mit Relinking und Gemini API-Aufruf
import time
import os
import cv2
...
def compare_reference_frames_with_gemini(old_frames, new_frames):
    ...
    contents = [
        """
        Vergleiche zwei Gruppen von Bildern einer verfolgten Person.

        Die ersten Bilder gehören zu einer alten Track-ID.
        Danach folgen Bilder einer neu erschienenen Track-ID.

        Beurteile, ob beide Gruppen wahrscheinlich dieselbe Person zeigen.

        Berücksichtige vor allem:
        - Kleidung
        - Farben und Muster
        - Hose
        - Schuhe
        - Taschen oder Accessoires
        - Kombination dieser sichtbaren Merkmale

        Perspektive, Haltung und teilweise Verdeckung dürfen unterschiedlich sein.
    ...
    # Alte Referenzbilder
    for reference in old_frames:
        image_bytes = crop_to_bytes(reference["crop"])
    ...
    contents.append(
        "Die folgenden Bilder gehören zur neuen Track-ID."
    )

    # Neue Referenzbilder
    for reference in new_frames:
        image_bytes = crop_to_bytes(reference["crop"])
...
def assign_target_id(track_id, person_box, current_histogram, frame_idx):
    ...
    if match is not None:
        ...
        vision_result = compare_reference_frames_with_gemini(
            old_reference_frames,
            new_reference_frames
        )

        print(
            f"Gemini-Vergleich BT {lost_track_id} -> "
